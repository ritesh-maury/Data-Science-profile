...
ipl_data.head() # First few data points
...
#Find the frequency of matches held in various cities 
...
# Mean of win by runs
np.mean(ipl_data['win_by_runs'])


# In[23]:


# Variance of win by runs
np.var(ipl_data['win_by_runs'])


# ## Winner after winning the toss

# In[28]:


## Toss winning team wining the Match
def winner_toss_winner(ipl_data):
    count=0
    for index,entry in ipl_data.iterrows():
...
result=winner_toss_winner(ipl_data)
total_matches=ipl_data.shape[0]
percentage=((result/total_matches)*100)
print(total_matches,result,percentage)


# In[30]:


print('Team winning toss has won ' + str(result) + ' times out of '+ str(total_matches) +' which  ' + str(("%.2f" % percentage))+'%') 


# ## Winner after winning the toss and electing to bat

# In[31]:


def winner_toss_winner_bat(ipl_data):
    ''' Toss winning and batting decision'''
    count_toss=0
    count_bat=0
    for index,entry in ipl_data.iterrows():
        ...
        if entry['toss_winner'] == entry['winner']:
            count_toss +=1
            if entry['toss_decision']=='bat':
                count_bat +=1
                
    result = (count_toss,count_bat)
    
    return result

result=winner_toss_winner_bat(ipl_data)

total=result[0]
bat_total=result[1]
percentage=((bat_total/total)*100)

print('Team winning toss and electing to bat has won ' + str(bat_total) + ' times out of '+ str(total) +' which is ' + str(("%.2f" % percentage))+'%')


# ### Addining the type of matches, such as final qualifyies, final etc
...
# ## Importing the delivery data
# This data contains bal by ball data of all the matches.
